...
//	data->height = data->mode->h;
	...
	data->window = SDL_CreateWindow("RTv1", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1920, 1080,  SDL_WINDOW_SHOWN);
	!(data->window) ? error(WINDOW_CREATE_ERROR) : 0;
	data->surface = SDL_GetWindowSurface(data->window);
	data->renderer = SDL_CreateRenderer(data->window, -1, SDL_RENDERER_ACCELERATED);
	data->texture = IMG_LoadTexture(data->renderer, "../textures/1.jpg");
	return (data);
}
