...
static	int		switch_camera(t_rt *data, int key)
{
	...
	data->current_camera = data->current_camera < 0 ?
			data->parse.camera_size - 1 :
...
void			keyboard_controller(SDL_Event *e, t_rt *data)
{
	...
	{
		data->update_status = e->key.keysym.sym == SDLK_p ?
				update_samples(data) : data->update_status;
		data->update_status = (e->key.keysym.sym == SDLK_LEFT
